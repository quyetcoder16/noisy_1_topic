...
def main():
    ...
    print(labels.head())
    print(f"data loaded, {labels.shape[0]} samples")
    ...
    for epoch in range(epochs):
        ...
        # Print results
        if epoch % 5 == 0:
            print("Epoch:", '%03d' % (epoch + 1), "train_loss=", "{:.5f}".format(np.mean(train_loss)), "val_loss=",
                  "{:.5f}".format(np.mean(val_loss)), "time=", "{:.5f}".format(time.time() - start))
    ...
    for i in range(0, len(testd)):
        ...
        fw.write(
            row["graph"] + "," + row["node"].replace(",", "-") + "," + str(len(row["node"].split(","))) + "," + str(
                o) + "," + str(y) + "\n")
